...
if (fs.existsSync(CERT_PATH)) {
  process.stdout.write(
    chalk.yellow.bold('Existing certificate found') + chalk.dim(' It will be replaced with a new one\n')
  );
...
if (fs.existsSync(CERT_KEY_PATH)) {
  process.stdout.write(
    chalk.yellow.bold('Existing private key found.') + chalk.dim(' It will be replaced with a new one\n')
  );
...
getDevelopmentCertificate('starter', { installCertutil: true })
  .then(({ key, cert }) => {
    process.stdout.write(chalk.green.bold('Success!/n'));
